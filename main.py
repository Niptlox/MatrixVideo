# https://github.com/StanislavPetrovV/Matrix-Vision
...
class Matrix:
    ...
    def get_prerendered_chars(self):
        char_colors = [(green, green, green) for green in range(256)]
        prerendered_chars = {}
    ...
    def draw(self):
        ...
        if pygame.key.get_pressed()[pg.K_x]:
            print([pg.Color(self.image[i, 100]) for i in range(400)])
        try:
...
class MatrixVideo:
    def __init__(self, path):
        ...
        pg.init()
        self.screen = pg.display.set_mode(self.RES)
        pygame.display.set_caption("Bad Apple matrix")
        ...
        # start video
        while True:
            if [i for i in pg.event.get() if i.type == pg.QUIT]:
                break

        self.video = Video(path)
        self.video.set_size(self.RES)
        self.video.play()
